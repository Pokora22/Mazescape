    ...
    private IEnumerator InitializeMaze()
    {
        ...
        for (int i = 0; i < maxPortals; i++)
        {
            if(i < redPortals.Length)
                redPortals[i].gameObject.GetComponent<scr_KeyData>().initializeKeyInWorld(keySpawns, randomKeyPlacement); //always one in green zone
            if (i < whitePortals.Length)
                whitePortals[i].gameObject.GetComponent<scr_KeyData>().initializeKeyInWorld(keySpawns, randomKeyPlacement); //always one in red zone
            if(i < bluePortals.Length)
                bluePortals[i].gameObject.GetComponent<scr_KeyData>().initializeKeyInWorld(keySpawns, randomKeyPlacement); //always one in white zone
